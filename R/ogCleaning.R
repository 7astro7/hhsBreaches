...
# tob : type of breach 
tob_discretized <- discretize(data_all, "type_of_breach")
# lobi: location of breached info
lobi_discretized <- discretize(data_all, "location_of_breached_information")

getOgData <- function() {
    return (data_all)
}

getParsedBreachFactors <- function(breach_types) {
    if (breach_types) {
        return (tob_discretized)
    }
    return (lobi_discretized)
}
